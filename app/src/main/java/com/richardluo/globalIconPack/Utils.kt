...
inline fun <T1 : Any, T2 : Any, R : Any> letAll(p1: T1?, p2: T2?, block: (T1, T2) -> R?): R? {
...
@Suppress("UNCHECKED_CAST")
fun <R> Field.getAs(thisObj: Any?): R {
  return get(thisObj) as R
}

@Suppress("UNCHECKED_CAST")
fun <R> Field.getAs(thisObj: Any?, block: (R) -> Unit) {
  block.invoke(get(thisObj) as R)
}
